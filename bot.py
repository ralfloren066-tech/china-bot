...
# ── Google Sheets ─────────────────────────────────────────────────────────────
HEADERS = ["№", "Дата", "Источник", "Ссылка", "Название", "Категория",
           "Цена (¥)", "Курс ¥→₽", "Цена (₽)", "Кол-во", "Размер", "Итого (₽)"]

CATEGORIES = ["Кроссовки", "Одежда", "Аксессуары", "Сумки", "Шапки / кепки", "Другое"]

def _get_ws():
    import json

    raw = CREDS_JSON.strip()
    # Убираем случайные лишние кавычки вокруг (Railway иногда добавляет)
    ...
    try:
        ...
        raise RuntimeError(
            f"GOOGLE_CREDS_JSON невалидный JSON: {e}\n\n"
            "Как исправить в Railway:\n"
            "1. Variables → GOOGLE_CREDS_JSON → удали значение\n"
            "2. Открой JSON-файл от Google в текстовом редакторе\n"
            "3. Ctrl+A → Ctrl+C (выдели и скопируй ВСЁ)\n"
            "4. Вставь в поле Railway БЕЗ лишних кавычек вокруг"
        ) from e
...
# Маркер строки ИТОГО — по этому тексту находим строку
TOTAL_MARKER = "ИТОГО"

def _find_total_row(ws) -> int | None:
    """Ищем строку с ИТОГО в колонке E (название). Возвращает номер строки (1-based) или None."""
    col_e = ws.col_values(5)  # колонка E = 5
    for i, val in enumerate(col_e, start=1):
        if val and TOTAL_MARKER in str(val):
            return i
    return None
...
def sheet_append(data: dict, rate: float) -> int:
    ...
    # Номер товара = кол-во строк с данными (без заголовка и строки ИТОГО)
    total_row_idx = _find_total_row(ws)
    if total_row_idx:
        num = total_row_idx - 1  # данных = строк до ИТОГО минус заголовок
    else:
        num = len(all_rows)  # нет строки ИТОГО — просто добавляем в конец
    ...
    if total_row_idx:
        # Вставляем строку ПЕРЕД строкой ИТОГО
        ws.insert_row(new_row, index=total_row_idx, value_input_option="USER_ENTERED")
    else:
...
def sheet_last(n=5):
    ...
    if len(rows) <= 1:
        return []
    data_rows = [r for r in rows[1:] if r and TOTAL_MARKER not in str(r[4])]
    return data_rows[-n:]
...
def kb_category():
    ...
    cats = [
        ("👟 Кроссовки",     "cat_Кроссовки"),
        ("👕 Одежда",        "cat_Одежда"),
        ("💍 Аксессуары",    "cat_Аксессуары"),
        ("👜 Сумки",         "cat_Сумки"),
        ("🧢 Шапки / кепки","cat_Шапки / кепки"),
        ("📦 Другое",        "cat_Другое"),
    ]
...
def confirm_text(st: dict) -> str:
    ...
    qty       = st["qty"]
    size      = st.get("size") or "—"
    category  = st.get("category") or "Другое"
    price_rub = round(price * rate, 2)
    total_rub = round(price_rub * qty, 2)
    name      = st.get("name") or "—"
...
async def on_callback(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "save":
        ...
        try:
            ...
            log.error(f"sheet_append error: {full_err}")
            # Показываем первые 300 символов ошибки — этого хватит для диагностики
            short = str(e)[:300]
            await q.edit_message_text(
                f"❌ Ошибка при сохранении:\n`{short}`\n\n"
                f"Проверь логи Railway для полного трейсбека.",
                parse_mode="Markdown"
            )
            return
        st_del(uid)
        ...
        st["step"] = "edit_cat_confirm"
        st_set(uid, st)
        await q.edit_message_text("🏷 Выбери новую категорию:", reply_markup=kb_category())

    elif action.startswith("cat_"):
        chosen_cat = action[4:]
        prev_step  = st.get("step", "")
        st["category"] = chosen_cat

        if prev_step == "edit_cat_confirm":
            # Редактируем из карточки подтверждения
            st["step"] = "confirm"
            st_set(uid, st)
            await q.edit_message_text(
                f"✅ Категория: *{chosen_cat}*", parse_mode="Markdown"
            )
            await ctx.bot.send_message(
                u.effective_chat.id, confirm_text(st),
                parse_mode="Markdown", reply_markup=kb_confirm(),
            ...
            # Обычный флоу — идём к размеру
            st["step"] = "size"
            st_set(uid, st)
            await q.edit_message_text(
                f"🏷 Категория: *{chosen_cat}*\n\n"
                f"📐 Введи размер (например: *42*, *L*, *One Size*)",
                parse_mode="Markdown",
