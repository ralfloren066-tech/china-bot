...
def _find_total_row(ws) -> int | None:
    """Ищем строку ИТОГО по всем колонкам. Возвращает номер строки (1-based) или None."""
    try:
        all_vals = ws.get_all_values()
        for i, row in enumerate(all_vals, start=1):
            for cell in row:
                if TOTAL_MARKER in str(cell):
                    return i
    except Exception:
        pass
    return None
...
def sheet_append(data: dict, rate: float) -> int:
    ...
    # Считаем порядковый номер товара
    if total_row_idx and total_row_idx > 1:
        num = total_row_idx - 1  # строк данных = строк до ИТОГО минус заголовок
    else:
        num = max(len(all_rows), 1)
    ...
    new_row = [
        ...
        str(data.get("category") or "Другое"),
        price,
        rate,
        price_rub,
        qty,
...
def sheet_last(n=5):
    ...
    if len(rows) <= 1:
        return []
    data_rows = [r for r in rows[1:] if r and TOTAL_MARKER not in str(r)]
    return data_rows[-n:]
...
async def on_callback(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "save":
        ...
        try:
            ...
            log.error(f"sheet_append error:\n{full_err}")
            # Берём последние строки трейсбека — там самое важное
