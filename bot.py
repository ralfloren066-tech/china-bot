...
# Домены которые считаем ссылкой на товар
URL_DOMAINS = [
    "dewu.com", "poizon.com", "du.com",
    "taobao.com", "tmall.com", "item.taobao",
    "1688.com",
    "95fen.com", "95.com",
]
...
def extract_url(update: Update) -> str | None:
    """
    Достаём URL из сообщения несколькими способами:
    1. Telegram entities (самый надёжный — бот видит реальный URL даже если текст обрезан)
    2. Регулярка по тексту (http/https)
    3. Домен без схемы (dewu.com/...)
    """
    msg = update.message

    # 1. Entities — Telegram сам парсит ссылки
    if msg.entities:
        for entity in msg.entities:
            if entity.type in ("url", "text_link"):
                url = entity.url if entity.type == "text_link" else \
...
def sheet_append(data: dict, rate: float) -> int:
    ...
    target = num + 1
    ws.format(f"F{target}:H{target}", {"numberFormat": {"type": "NUMBER", "pattern": "#,##0.00"}})
    ws.format(f"K{target}",           {"numberFormat": {"type": "NUMBER", "pattern": "#,##0.00"}})
    return num
...
def allowed(uid: int) -> bool:
    ...
    if not result:
        log.warning(f"Отклонён пользователь uid={uid}, разрешён только uid={ALLOWED_UID}")
    return result
...
def kb_price_actions():
    return InlineKeyboardMarkup([[InlineKeyboardButton("✏️ Ввести вручную", callback_data="manual_price")]])
...
async def cmd_whoami(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Показывает твой Telegram ID — полезно для диагностики."""
    uid = u.effective_user.id
    await u.message.reply_text(
        f"🪪 Твой Telegram ID: `{uid}`\n"
        f"ALLOWED\\_USER\\_ID в боте: `{ALLOWED_UID}`\n"
        f"Доступ: {'✅ разрешён' if allowed(uid) else '❌ запрещён'}",
        parse_mode="Markdown"
    )
...
async def on_message(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if url:
        ...
        # Если цена нашлась — показываем её и просим подтвердить / идём дальше
        if parsed.get("price"):
            state["step"] = "qty"
            st_set(uid, state)
    ...
    await u.message.reply_text("👟 Отправь ссылку на товар чтобы начать.\n/cancel — отмена ввода.")
